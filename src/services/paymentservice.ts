...
export class PaymentService {
  ...
  /**
   * Generate unique payment reference with FULL userId
  ...
  private generateReference(
    ...
    // DO NOT truncate userId - we need the full ID for lookups
    const timestamp = Date.now();
    const randomSuffix = Math.random().toString(36).substring(2, 8);
    return `${gateway}_${paymentType}_${timestamp}_${userId}_${randomSuffix}`;
  }
  ...
  async initializePayment(
    ...
    try {
      ...
      // Generate unique reference with FULL userId
      const reference = this.generateReference(gateway, paymentType, userId);

      console.log(`💳 Initializing ${gateway} payment: ${reference}`);
      ...
      console.log(`✅ Payment record created in DB: ${reference}`);
  ...
  private async initializePaystackPayment(
    ...
    try {
      ...
      console.log(
        `🟢 Initializing Paystack: ${amountInKobo} kobo (${amountUSD} USD)`
      );
      ...
      const response = await axios.post(
        `${PAYSTACK_BASE_URL}/transaction/initialize`,
        {
          ...
          currency: "NGN",
          metadata: {
            userId, // Store full userId in metadata
            paymentType,
            custom_fields: [
              {
                display_name: "Payment Type",
      ...
      console.log(
        `✅ Paystack session created: ${response.data.data.access_code}`
      );
  ...
  async handleStripeWebhook(event: Stripe.Event): Promise<void> {
    ...
    try {
      ...
      // Don't throw - log and continue
      // This prevents webhook retry loops for unrecoverable errors
    }
  }
  ...
  async getUserPaymentStatus(userId: string) {
    try {
      ...
      console.error(`❌ Error getting user payment status: ${error.message}`);
      throw new Error("Failed to get user payment status");
    }
  }
